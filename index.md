<div data-lang-content="en" data-active-lang markdown="1">

<div data-page-content="about" data-active-page markdown="1">

<a id="about"></a>
# ABOUT ME

I am interested in robotics, intelligent systems, product design, and the way engineering research can become practical tools for real people. My academic path in mechanical engineering and aeronautical engineering has shaped how I think: start from a real problem, understand the physical system, build carefully, test honestly, and keep improving.

I hope to keep working at the intersection of mechanical design, sensing, AI, and human-centered products. I value curiosity, disciplined execution, and projects that connect technical depth with meaningful everyday impact.

</div>

<div data-page-content="education" markdown="1">

<a id="education"></a>
# EDUCATION
<big>**2022/08-**: [National University of Singapore](https://www.nus.edu.sg/)</big>

- Major: Mechanical Engineering
- Master of Science

<big>**2021/09-2022/07**: [Shenzhen innoX Academy](https://innoxsz.com/)</big>

- Courses jointly taught by Hong Kong University of Science and Technology, Southern
University of Science and Technology, Cheung Kong Graduate School of Business and
industrial experts: Robotic System Design, Internet of Things and Mobile Awareness,
Basic Sensor Principle and Technology, Design Thoughts, Entrepreneurship, etc.
- On-site visits to industrial plants: Sinocare Healthcare, Hai Robotics, Pony.AI, etc.
- Supervised research: an intelligent wheelchair with aided-driving technology (undergoing)
- Full financial support

<big>**2017/09-2021/07** : [Sun Yat-sen University](https://www.sysu.edu.cn/sysuen/)</big>

- Major: Aeronautical and Astronautical Engineering
- Bachelor of Engineering
- Cumulative GPA: 85/100
- Scholarship based on Outstanding Academic Performance for 3 consecutive years

## Research, Competitions & Campus Work

# PUBLICATION & PATENT
- Wei Zhang, **Ziye Hong**, Jianing Wu, Zhigang Wu. A Bionic Comb Device for Cleaning Flexible Column. CN202010846745.5 November 2020
- **Ziye Hong**. Prediction of 2019-nCoV Epidemic by Linear Regression Model. *ACM ICPS*. ISBN: 978-1-4503-8860-3, July 2020
- Chihang Liu, Hao Luo, Hui Cheng, **Ziye Hong**, Runtao Liu. A novel chassis-detachable storage and output sphere. CN202020242466.3 March 2020

# PROJECTS

<big>**2021/07-2021/08** : [XbotPark](http://www.xbotpark.com/en/) 2021 Intelligent C-Terminal Scientific Innovation Camp</big>

*Supervisor: Prof. [Zexiang Li](https://seng.hkust.edu.hk/about/people/faculty/zexiang-li) from Hong Kong University of Science and Technology*

* Learned the fundamental courses of product design and visited the robot production lines at DJI Innovations, 3irobotix, etc.
* Designed independently and produced a safe fishing-aid bait adding solution.
* Earned 2nd prize in the summer camp.

<big>**2021/01-2021/06** : Advanced Technology Research Institute MicroLab of Sun Yat-sen University</big>

*Supervisor: Prof. [Tianjiang Hu](https://saa.sysu.edu.cn/teacher/186) from Sun Yat-sen University*

* Achieved target detection on the UAV fuselage key points by convolutional neural network (modified YOLO algorithm) in Linux.
* Recorded the sequence charts of the UAV flight tests and obtained the labeled dataset with LabelImg.
* Adopted two attitude estimation algorithms based on PnP to compare accuracy and speed: EPnP method and POSIT iterative algorithm.
* Summarized the findings in academic writing and earned the honor of "Excellent Thesis Work".

<big>**2019/06-2020/01** : Intelligent Bionic Robot Lab of Sun Yat-sen University</big>

*Supervisor: Prof. [Zhigang Wu](https://saa.sysu.edu.cn/teacher/196) and Associate Prof. [Jianing Wu](https://saa.sysu.edu.cn/teacher/233) from Sun Yat-sen University*

* Detected the bee's tentacle-cleaning behavior via high-speed camera, observed the secondary structure, and measured structural parameters with a confocal laser scanning microscope.
* Designed and manufactured the cleaning structure in SolidWorks and with a 3D printer, then measured friction force and cleaning efficiency with a sensor.
* Concluded that the cleaning structure with secondary structure has greater friction force and higher cleaning efficiency.

<big>**2020/02-2020/06** : Prediction of 2019-nCoV Epidemic by Linear Regression Model</big>

*Supervisor: Prof. [Rakesh Kumar](https://cs.illinois.edu/about/people/all-faculty/rakeshk) from University of Illinois at Urbana-Champaign*

* Attended the online course on Artificial Intelligence.
* Adopted Python to glean data about the 2019-nCoV epidemic from January to February, such as the number of infected patients, deaths, infection rate, and mortality, and visualized the data by province.
* Predicted the mortality of different provinces via a linear regression model learned from the course, improved the existing model by using mortality as a second variable, and obtained more accurate predictions.
* Conducted academic writing and presented the work on ACM ICPS.

# CONTESTS

<big>**2019/05-2019/11** : [MathWorks Minidrone Competition](https://ww2.mathworks.cn/academia/student-competitions/minidrones/sysu-2019.html?s_tid=srchtitle_MathWorks%20Minidrone%20Competition%20SYSU_1)</big>  *<u>Chief Member</u>*

* Received systematic training in UAV control systems and image recognition technology.
* Programmed in MATLAB SIMULINK to carry out image recognition.

<big>**2019/05-2019/11** : [RoboMaster University Championship (RMUC)](https://www.robomaster.com/en-US)</big>  *<u>Chief Member</u>*

* Served as chief engineer responsible for mechanical design of the robot via SolidWorks, Ansys structural analysis, 3D printing, metal processing, and electronic-control testing.
* Led the team to win multiple honors in national contests, including *Superior Appearance Design*, conducted academic writing, and obtained patent authorization.

<big>**2018/10-2019/04** : Software Innovation Contest of SYSU</big>  *<u>Chief Member</u>*

* Developed a WeChat mini program with the WeChat developer toolkit and stored data on Ali Cloud server.
* Achieved image transmission from UAV to users' mini program through TeamViewer.
* Earned 2nd prize in the university contest and the Best Popularity Award.

<big>**2018/01-2018/06** : The Challenge Cup Business Plan Contest for National College Student</big>  *<u>Team Leader</u>*

* Proposed the innovation of shared UAV for aerial photography and formed a team to carry out the project.
* Designed the overall business plan and demonstrated the proposed software product.
* Earned 3rd Prize.

<big>**2019/05-2019/11** : The Dynamic Analysis of Writing Brush Furcation Phenomenon Based on Experiments</big>  *<u>Chief Member</u>*

*Supervisor: Prof. [Qinghe Yao](https://saa.sysu.edu.cn/teacher/238) and Dr. Wenhao Cai from Sun Yat-sen University*

* Established a physical model and carried out verification experiments on writing brush furcation.
* Visualized the experimental data in Matlab and verified the model.
* Summarized the finding in academic writing and published on the Yau Mathematical Sciences Center, Tsinghua University.
* Earned the Global Gold Award in the Shing-Tung Yau Physics Contest for Middle School Students.

</div>

<div data-page-content="work" markdown="1">

<a id="work"></a>
# WORK EXPERIENCE

Work experience will be added here. Suggested items include internship or full-time role, organization, location, dates, key responsibilities, tools used, and measurable outcomes.

</div>

</div>

<div data-lang-content="zh" markdown="1">

<div data-page-content="about" data-active-page markdown="1">

<a id="about"></a>
# 关于我

我关注机器人、智能系统、产品设计，以及工程研究如何转化为真正可用、能帮助人的工具。机械工程与航空航天工程的学习经历塑造了我的思考方式：从真实问题出发，理解物理系统，谨慎构建，诚实测试，并持续迭代。

我希望继续在机械设计、传感、人工智能与以人为中心的产品之间探索。对我来说，长期的兴趣来自好奇心、扎实执行，以及把技术深度和现实价值连接起来的项目。

</div>

<div data-page-content="education" markdown="1">

<a id="education"></a>
# 教育背景
<big>**2022/08-**：[新加坡国立大学](https://www.nus.edu.sg/)</big>

- 专业：机械工程
- 理学硕士

<big>**2021/09-2022/07**：[深圳科创学院](https://innoxsz.com/)</big>

- 课程由香港科技大学、南方科技大学、长江商学院及产业专家联合授课，内容包括机器人系统设计、物联网与移动感知、传感器原理与技术、设计思维、创业等。
- 参加产业现场参访：三诺医疗、海柔创新、小马智行等。
- 参与导师指导研究：具备辅助驾驶技术的智能轮椅（进行中）。
- 获全额资助。

<big>**2017/09-2021/07**：[中山大学](https://www.sysu.edu.cn/sysuen/)</big>

- 专业：航空航天工程
- 工学学士
- 累计 GPA：85/100
- 连续 3 年获得优秀学业奖学金

## 科研、竞赛与校园项目

# 论文与专利
- Wei Zhang, **Ziye Hong**, Jianing Wu, Zhigang Wu. A Bionic Comb Device for Cleaning Flexible Column. CN202010846745.5 November 2020
- **Ziye Hong**. Prediction of 2019-nCoV Epidemic by Linear Regression Model. *ACM ICPS*. ISBN: 978-1-4503-8860-3, July 2020
- Chihang Liu, Hao Luo, Hui Cheng, **Ziye Hong**, Runtao Liu. A novel chassis-detachable storage and output sphere. CN202020242466.3 March 2020

# 项目经历

<big>**2021/07-2021/08**：[XbotPark](http://www.xbotpark.com/en/) 2021 智能 C 端科技创新营</big>

*导师：[香港科技大学李泽湘教授](https://seng.hkust.edu.hk/about/people/faculty/zexiang-li)*

* 学习产品设计基础课程，并参访大疆创新、3irobotix 等机器人生产线。
* 独立设计并制作安全型钓鱼辅助添饵方案。
* 获夏令营二等奖。

<big>**2021/01-2021/06**：中山大学先进技术研究院 MicroLab</big>

*导师：[中山大学胡天江教授](https://saa.sysu.edu.cn/teacher/186)*

* 在 Linux 系统中使用卷积神经网络（改进 YOLO 算法）实现无人机机身关键点目标检测。
* 记录无人机飞行测试序列图，并使用 LabelImg 获得标注数据集。
* 采用两种基于 PnP 的姿态估计算法比较精度与速度：EPnP 方法和 POSIT 迭代算法。
* 完成学术写作总结，并获“优秀毕业论文”荣誉。

<big>**2019/06-2020/01**：中山大学智能仿生机器人实验室</big>

*导师：[中山大学吴志刚教授](https://saa.sysu.edu.cn/teacher/196)及[吴嘉宁副教授](https://saa.sysu.edu.cn/teacher/233)*

* 通过高速相机检测蜜蜂触角清洁行为，进一步观察二级结构，并用共聚焦激光扫描显微镜测量结构参数。
* 使用 SolidWorks 和 3D 打印设计制造清洁结构，并通过传感器测量摩擦力和清洁效率。
* 得出具有二级结构的清洁结构拥有更大摩擦力和更高清洁效率的结论。

<big>**2020/02-2020/06**：基于线性回归模型的 2019-nCoV 疫情预测</big>

*导师：[伊利诺伊大学厄巴纳-香槟分校 Rakesh Kumar 教授](https://cs.illinois.edu/about/people/all-faculty/rakeshk)*

* 参加人工智能在线课程。
* 使用 Python 收集 2019-nCoV 疫情 1 月至 2 月的数据，包括感染人数、死亡人数、感染率、死亡率等，并按省份进行可视化。
* 使用课程中学习的线性回归模型预测不同省份死亡率，并将已有死亡率作为第二变量改进模型，获得更准确预测。
* 完成学术写作并在 ACM ICPS 进行展示。

# 竞赛经历

<big>**2019/05-2019/11**：[MathWorks Minidrone Competition](https://ww2.mathworks.cn/academia/student-competitions/minidrones/sysu-2019.html?s_tid=srchtitle_MathWorks%20Minidrone%20Competition%20SYSU_1)</big>  *<u>核心成员</u>*

* 系统接受无人机控制系统与图像识别技术训练。
* 使用 MATLAB SIMULINK 编程实现图像识别。

<big>**2019/05-2019/11**：[RoboMaster 机甲大师高校系列赛（RMUC）](https://www.robomaster.com/en-US)</big>  *<u>核心成员</u>*

* 担任机械组负责人，负责使用 SolidWorks、Ansys 结构分析、3D 打印、金属加工等完成机器人机械设计，并参与电控测试。
* 带领团队在全国赛事中获得多项荣誉，包括 *最佳外观设计*，完成学术写作并获得专利授权。

<big>**2018/10-2019/04**：中山大学软件创新竞赛</big>  *<u>核心成员</u>*

* 使用微信开发者工具开发微信小程序，并通过阿里云服务器存储数据。
* 通过 TeamViewer 实现无人机图像向用户小程序端传输。
* 获校级竞赛二等奖及最佳人气奖。

<big>**2018/01-2018/06**：挑战杯全国大学生创业计划竞赛</big>  *<u>团队负责人</u>*

* 提出共享无人机航拍创新方案，并组队推进项目。
* 设计整体商业计划并演示拟开发的软件产品。
* 获三等奖。

<big>**2019/05-2019/11**：基于实验的毛笔分叉现象动力学分析</big>  *<u>核心成员</u>*

*导师：[中山大学姚清河教授](https://saa.sysu.edu.cn/teacher/238)及蔡文浩博士*

* 建立物理模型，并开展毛笔分叉现象验证实验。
* 使用 Matlab 可视化实验数据并验证模型。
* 完成学术写作总结，并发表于清华大学丘成桐数学科学中心。
* 获丘成桐中学物理奖全球金奖。

</div>

<div data-page-content="work" markdown="1">

<a id="work"></a>
# 工作经历

工作经历可在这里补充。建议包含实习或全职岗位、机构名称、地点、时间、主要职责、使用工具，以及可以量化的成果。

</div>

</div>
